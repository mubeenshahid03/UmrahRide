...
router.post("/addvehicle",fetchuser,async(request,response)=>{
    try {
        ...
        let vehicle=new Vehicle({name,imgURL,cartype,seats,bags,price,userid});
...
router.post("/deletevehicle/:id",fetchuser,async(request,response)=>{
    try {
        ...
        {
            return response.status(200).json({message:`vehicle with id ${id} deleted successsfully `})
        }
...
router.put("/editvehicle/:id",fetchuser,async(request,response)=>{
    try {
        ...
        if(!request.params.id){
            ...
            const updatedVehicle= await Vehicle.findByIdAndUpdate(request.params.id,{$set:newVehicle},{new:true})
                if(!updatedVehicle){
...
router.get("/filtervehicles/:name",fetchuser,async(request,response)=>{
    try {
        ...
        console.log("error from backend /api/vehicles/filtervehicles/:name" + error)
    }
})
